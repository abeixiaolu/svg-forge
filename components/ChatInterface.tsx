    ...
    <div className="flex flex-col h-full overflow-hidden font-mono text-sm relative">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6 scrollbar-thin">
        ...
        {messages.map((msg) => (
          ...
          >
            <span className={`text-[10px] uppercase tracking-wider mb-1 px-1 ${msg.role === 'user' ? 'text-primary' : 'text-tech-dim'}`}>
                {msg.role === 'user' ? 'USER_CMD' : 'SYS_RESP'} // {new Date(msg.timestamp).toLocaleTimeString([], {hour12: false, hour:'2-digit', minute:'2-digit', second:'2-digit'})}
            </span>
            <div
            ...
            >
              <pre className="whitespace-pre-wrap font-mono text-xs leading-relaxed font-sans">{msg.content}</pre>
            </div>
          </div>
        ))}
        ...
        {isLoading && (
          <div className="flex flex-col items-start gap-1 animate-pulse">
             <span className="text-[10px] text-primary uppercase tracking-wider mb-1 px-1">SYS_PROC</span>
